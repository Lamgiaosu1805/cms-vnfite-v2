...
function ApproveModal({ loan, onConfirm, onCancel }: ApproveModalProps) {
  const [rate, setRate] = useState('');
  const [notes, setNotes] = useState('');
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full">
        <h3 className="font-semibold text-gray-800 mb-4">Phê duyệt khoản vay</h3>
        ...
        <p className="text-sm text-gray-500 mb-4">
          Nhập lãi suất thẩm định (%/năm) — sẽ gửi cho người vay xác nhận.
        </p>
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button
            onClick={() => onConfirm(parseFloat(rate), notes)}
...
function RejectModal({ loan, onConfirm, onCancel }: RejectModalProps) {
  const [reason, setReason] = useState('');
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full">
        <h3 className="font-semibold text-gray-800 mb-3">Từ chối khoản vay #{loan.loanId}</h3>
        <textarea
          value={reason}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-5 py-3.5 font-medium text-gray-600">Khoản vay</th>
                <th className="text-left px-4 py-3.5 font-medium text-gray-600">Số tiền</th>
                <th className="text-left px-4 py-3.5 font-medium text-gray-600">Lãi suất</th>
                <th className="text-left px-4 py-3.5 font-medium text-gray-600">Kỳ hạn</th>
            ...
            <tbody className="divide-y divide-gray-50">
                ...
                <tr key={loan.loanId} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3.5">
                    <p className="font-medium text-gray-900 max-w-[200px] truncate">
                      {loan.purpose || `Khoản vay #${loan.loanId}`}
                    </p>
                    <p className="text-xs text-gray-400">
                      {loan.borrowerId}
                    </p>
                  </td>
                  <td className="px-4 py-3.5 text-gray-700 font-medium">{formatMoney(loan.amount)}</td>
                  <td className="px-4 py-3.5 text-gray-600">
                    {loan.interestRate != null ? `${loan.interestRate}%` : <span className="text-gray-400 text-xs">Chưa thẩm định</span>}
                  </td>
                  <td className="px-4 py-3.5 text-gray-600">{loan.termMonths} tháng</td>
                  <td className="px-4 py-3.5"><Badge value={loan.status} /></td>
                  <td className="px-4 py-3.5 text-gray-400">{formatDate(loan.createdAt)}</td>
              ...
              {data?.content.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-8 text-center text-gray-400">Không có khoản vay nào</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {data && data.totalPages > 1 && (
          <div className="px-5 py-3.5 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
            <span>Tổng {data.totalElements} khoản vay</span>
      ...
      {approveModal && (
        <ApproveModal
          loan={approveModal}
          onConfirm={(rate, notes) => {
            setApproveModal(null);
            doAction(approveLoan(approveModal.loanId, rate, notes));
          }}
      ...
      {rejectModal && (
        <RejectModal
          ...
          onConfirm={(reason) => {
            setRejectModal(null);
            doAction(rejectLoan(rejectModal.loanId, reason));
          }}
